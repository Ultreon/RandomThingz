...
public class StandingSignFrameBlock extends StandingSignBlock {
    public StandingSignFrameBlock(Properties properties) {
        ...
        this.setDefaultState(this.stateContainer.getBaseState().with(ROTATION, 0).with(WATERLOGGED, Boolean.FALSE).with(BCBlockStateProperties.CONTAINS_BLOCK, false).with(BCBlockStateProperties.LIGHT_LEVEL, 0));
    }

    @Override
    protected void fillStateContainer(StateContainer.Builder<Block, BlockState> builder) {
        builder.add(ROTATION, WATERLOGGED, BCBlockStateProperties.CONTAINS_BLOCK, BCBlockStateProperties.LIGHT_LEVEL);
    }
