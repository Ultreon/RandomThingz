 ...
 *
 * @author Qboi123
 * @deprecated Use {@linkplain MoobloomFlowerLayer} instead.
 */
@Deprecated
@OnlyIn(Dist.CLIENT)
public class MoobloomFlowerLayerOld<T extends MoobloomEntityOld> extends RenderLayer<T, CowModel<T>> {
    public MoobloomFlowerLayerOld(RenderLayerParent<T, CowModel<T>> rendererIn) {
        super(rendererIn);
    }
    ...
    public void render(@NotNull PoseStack matrixStackIn, @NotNull MultiBufferSource bufferIn, int packedLightIn, @SuppressWarnings("SpellCheckingInspection") T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (!entitylivingbaseIn.isBaby() && !entitylivingbaseIn.isInvisible()) {
            ...
            BlockState blockstate = ModBlocks.BUTTERCUP.get().defaultBlockState();
