...
public abstract class DeviceItem extends HudItem {
    ...
    public DeviceItem(Properties properties, int maxEnergy, int maxTransfer) {
        this(properties, maxEnergy, maxTransfer, maxTransfer);
    }

    public DeviceItem(Properties properties, int maxEnergy, int maxReceive, int maxExtract) {
        super(properties);
    ...
    public void renderHud(PoseStack pose, GraphicsUtil gu, Minecraft mc, ItemStack stack, LocalPlayer player) {
        ...
        stack.getCapability(CapabilityEnergy.ENERGY).ifPresent(e -> {
            ...
            gu.drawItemStack(RenderSystem.getModelViewStack(), stack, 56, height - 60, "");
