...
public class ServerScriptInstance extends AbstractScriptInstance {
    private final ServerPlayerEntity player;

    private static final Logger LOGGER = LogManager.getLogger("QFM:Script:Server");

    ServerScriptInstance(Language language, ServerPlayerEntity player, Context engine) {
        super(language, engine, Dist.DEDICATED_SERVER);
        this.player = player;
        init();
    }
    ...
    protected void init() {
        ...
        bindings.putMember("world", (Supplier<ServerWorld>) () -> new ServerWorld(player.getServerWorld()));
        bindings.putMember("pos", (Supplier<BlockPos>) player::getPosition);

        bindings.putMember("System", new ServerSystemOverride(getLanguage(), player));
        try {
            // General classes
            if (QForgeMod.isClientSide()) {
