...
import com.qsoftware.forgemod.script.ScriptJSFormatter;
...
public class ScriptJSScreen extends Screen {
   ...
   private static Dist executeDist = Dist.DEDICATED_SERVER;
   ...
   private ScriptJSScreen(String defaultText) {
...
//      this.textLock = false;
   ...
   public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
      ...
      /*if (this.commandSuggestionHelper.onKeyPressed(keyCode, scanCode, modifiers)) {
         ...
         if (!s.isEmpty()) {
            if (executeDist == Dist.DEDICATED_SERVER) {
               Network.channel.sendToServer(new ScriptPacket(Language.JS, s));
            } else if (executeDist == Dist.CLIENT) {
               ScriptManager.getClientInstance().eval(Language.JS, s).ifLeft((e) -> {
                  e.printStackTrace();
...
//            this.commandSuggestionHelper.shouldAutoSuggest(false);
   ...
   public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
      ...
      RenderSystem.translatef(0.0F, (float)(this.height - 52), 0.0F);
