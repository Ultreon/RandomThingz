...
public class ServerScriptJSInstance extends AbstractScriptJSInstance {
    private final ServerPlayerEntity player;
    @Getter private final ScriptEngine engine;
    @Getter private final ScriptJS scriptJS;

    private static final Logger LOGGER = LogManager.getLogger("QFM:ScriptJS:Server");

    ServerScriptJSInstance(ServerPlayerEntity player, ScriptEngine engine) {
        this.player = player;
        this.engine = engine;
        this.scriptJS = new ScriptJS(this);

        this.engine.put("player", new ServerPlayer(player.getUniqueID()));
        ...
        this.engine.put("world", (Supplier<World>) () -> new World(player.getServerWorld()));
        this.engine.put("pos", (Supplier<BlockPos>) player::getPosition);

        this.engine.put("System", new ServerSystemOverride(player));
        try {
            // General classes
            if (QForgeMod.isClientSide()) {
