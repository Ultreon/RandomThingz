package com.qsoftware.silent.lib.command.internal;
...
public class DisplayNBTCommand {
    ...
    private static int runForBlock(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        context.getSource().sendErrorMessage(new TranslationTextComponent("command.qsilentlib.nbt.notBlockEntity", title));
        return 0;
    }
    ...
    private static int runForItem(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        if (stack.isEmpty()) {
            context.getSource().sendErrorMessage(new TranslationTextComponent("command.qsilentlib.nbt.noItemInHand"));
            return 0;
        } else if (!stack.hasTag()) {
            context.getSource().sendErrorMessage(new TranslationTextComponent("command.qsilentlib.nbt.noItemTag", stack.getDisplayName()));
            return 0;
        }
