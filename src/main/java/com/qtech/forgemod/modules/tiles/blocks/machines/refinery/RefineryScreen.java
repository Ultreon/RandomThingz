package com.qtech.forgemod.modules.tiles.blocks.machines.refinery;

import com.mojang.blaze3d.matrix.MatrixStack;
import com.qtech.forgemod.QForgeMod;
import com.qtech.forgemod.util.render.RenderUtils;
import com.qtech.forgemod.modules.tiles.blocks.machines.AbstractMachineBaseScreen;
...
public class RefineryScreen extends AbstractMachineBaseScreen<RefineryContainer> {
    ...
    protected void renderHoveredTooltip(MatrixStack matrixStack, int x, int y) {
        if (isPointInRegion(28, 17, 13, 51, x, y)) {
            renderTooltip(matrixStack, TextUtils.fluidWithMax(container.getFluidInTank(0), RefineryTileEntity.TANK_CAPACITY), x, y);
        }
        if (isPointInRegion(68, 17, 13, 51, x, y)) {
            renderTooltip(matrixStack, TextUtils.fluidWithMax(container.getFluidInTank(1), RefineryTileEntity.TANK_CAPACITY), x, y);
        }
        if (isPointInRegion(84, 17, 13, 51, x, y)) {
            renderTooltip(matrixStack, TextUtils.fluidWithMax(container.getFluidInTank(2), RefineryTileEntity.TANK_CAPACITY), x, y);
        }
        if (isPointInRegion(100, 17, 13, 51, x, y)) {
            renderTooltip(matrixStack, TextUtils.fluidWithMax(container.getFluidInTank(3), RefineryTileEntity.TANK_CAPACITY), x, y);
        }
        if (isPointInRegion(116, 17, 13, 51, x, y)) {
            renderTooltip(matrixStack, TextUtils.fluidWithMax(container.getFluidInTank(4), RefineryTileEntity.TANK_CAPACITY), x, y);
        }
