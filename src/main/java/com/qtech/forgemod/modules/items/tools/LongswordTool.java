...
import com.qtech.forgemod.modules.items.objects.tools.types.LongswordItem;
import com.qtech.forgemod.modules.items.tools.trait.AbstractTrait;
import com.qtech.forgemod.util.ItemUtils;
...
public class LongswordTool extends LongswordItem implements ITool {
    private final Supplier<AbstractTrait[]> traits;
    private final float attackDamage;
    private final Multimap<Attribute, AttributeModifier> toolAttributes;

    public LongswordTool(IItemTier tier, int attackDamageIn, float attackSpeedIn, Properties builderIn, Supplier<AbstractTrait[]> traits) {
    ...
    public Set<ToolType> getQfmToolTypes() {
        return new HashSet<>(Arrays.asList(ToolType.SWORD));
    }
    ...
    public boolean onLeftClickEntity(ItemStack stack, PlayerEntity player, Entity clicked) {
        ...
        for (AbstractTrait trait : getTraits()) {
            ...
            if (smiteValue < 0.0f) {
                QForgeMod.LOGGER.warn("Smite value is less that zero, this can cause weird behavior");
            }
